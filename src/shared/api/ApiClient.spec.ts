...
describe("ApiClient", () => {
  ...
  describe("getIteration", () => {
    it("returns root items with children attached", async () => {
      const fetchSpy = createApiMock({
        wiql: {
          workItemRelations: [
        ...
        batchFields: {
          value: [
            { id: 101, fields: { ...commonFields, "System.Id": 101, "System.Title": "Parent PBI", "System.WorkItemType": "Product Backlog Item", "Microsoft.VSTS.Scheduling.Effort": 8 } },
            { id: 102, fields: { ...commonFields, "System.Id": 102, "System.Title": "Child Task 1", "System.WorkItemType": "Task", "System.State": "Done" } },
            { id: 103, fields: { ...commonFields, "System.Id": 103, "System.Title": "Child Task 2", "System.WorkItemType": "Task", "System.State": "In Progress" } }
          ]
        },
        batchRelations: {
          value: [
            { id: 102, relations: [{ rel: "System.LinkTypes.Hierarchy-Reverse", url: "http://vso/parent/101" }] },
      ...
      const client = new ApiClient("https://dev.azure.com/org", fetchSpy);
      const result = await client.getIteration("coll", "proj", "team", "Sprint 13");

      expect(result.workItems).toHaveLength(1);
      const root = result.workItems[0];
      expect(root).toBeDefined();
      expect(root?.title).toBe("Parent PBI");
      expect(root?.id).toBe(101);
      expect(root?.tasks).toHaveLength(2);
      expect(root?.tasks[0]?.System.Title).toBe("Child Task 1");
      expect(root?.tasks[1]?.System.Title).toBe("Child Task 2");
      expect(result.sprintStartDate?.toISOString()).toBe("2026-01-13T00:00:00.000Z");
      expect(result.sprintEndDate?.toISOString()).toBe("2026-01-26T00:00:00.000Z");
    });

    it("preserves items without parents and excludes orphan tasks with parents", async () => {
      const fetchSpy = createApiMock({
        wiql: {
          workItemRelations: [
            { source: { id: 101 }, target: { id: 102 } },
