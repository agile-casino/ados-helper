...
- **Hard-coded customer logic in shared code** — `getTeamAreaPath` falls back to a path containing `.replace("Pixel_Perfect", "PixelPerfect")` (`ApiClient.ts:78`) and `project === "WirelineRnD"` branches (`ApiClient.ts:87`, `CurrentTeamTab.tsx:56`). For a distributable marketplace extension these belong in configuration, not source. (Also flagged in `REVIEW_RECOMMENDATIONS.md`.)
